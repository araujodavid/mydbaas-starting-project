...
import main.java.br.com.arida.ufc.mydbaas.agent.entity.StatementMetric;
 ...
 * @since July 10, 2013
 */
public class StatementCollector extends AbstractCollector<StatementMetric> {
	...
	public StatementCollector(int identifier, String type) {
	...
	public void loadMetric(Object[] args) throws NumberFormatException, ClassNotFoundException, SQLException {
		...
		if (firstCycle == true) {
			...
			this.metric.setStatementInserts(0);
			this.metric.setStatementUpdates(0);
			this.metric.setStatementSelects(0);
			this.metric.setStatementDeletes(0);
		//Otherwise: the new value is subtracted by the value stored and the result is set in the metric
		} else {
			while (resultSet != null && resultSet.next()) {
				switch (resultSet.getString("Variable_name")) {
				case "Com_insert":
					this.metric.setStatementInserts(resultSet.getInt("Value") - inserts);
					inserts = resultSet.getInt("Value");
					break;
				case "Com_select":
					this.metric.setStatementSelects(resultSet.getInt("Value") - selects);
					selects = resultSet.getInt("Value");
					break;
				case "Com_update":
					this.metric.setStatementUpdates(resultSet.getInt("Value") - updates);
					updates = resultSet.getInt("Value");
					break;
				case "Com_delete":
					this.metric.setStatementDeletes(resultSet.getInt("Value") - deletes);
					deletes = resultSet.getInt("Value");
					break;
				}
	        }
	...
	@Override
	public void run() {
		this.metric = StatementMetric.getInstance();
		...
		if (this.metric.getDBMSs().length > 0) {
			for (String dbms : this.metric.getDBMSs()) {
				...
				try {
					...
					System.out.println("Problem loading the Statement metric value (DBMS)");
					e.printStackTrace();
				}
				...
				try {
					...
					if (response.getStatusLine().getStatusCode() != 202) {
						System.out.println("Statement request error!");
